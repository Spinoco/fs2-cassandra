...
trait DockerCassandra
  ...
  override protected def beforeEach(): Unit = LOGCALL {
    LOG("super1")
    super.beforeEach()
    LOG("super2")
...
object DockerCassandra {
  ...
  def cleanupSchema(cs:CassandraSession[IO], cassandra:CassandraDefinition)(preserveKeysSpace: String => Boolean):Unit = LOGCALL{
    cs.queryAll(cassandra.allKeySpaceQuery)
      ...
      .flatMap { n =>
        ...
        eval{
          LOG(s"Drop $n")
          cs.executeCql(s"DROP KEYSPACE $n")
        }
      }
