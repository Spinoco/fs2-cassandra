...
case class TableBuilder[R <: HList, PK <: HList, CK <: HList, IDX <: HList](
  ...
  :TableBuilder[R, P.Out, CK, IDX] = TableBuilder(ks, indexes)
  ...
  :TableBuilder[R,  PK, P.Out, IDX] = TableBuilder(ks, indexes)

  /** registers given `V` as column of this table with name `name` **/
  def column[K,V](name:Witness.Aux[K])(implicit ev:CType[V])
  : TableBuilder[FieldType[K,V] :: R, PK, CK, IDX] = TableBuilder(ks, indexes)
  ...
  def indexBy[K,V](column:Witness.Aux[K], name:String, clazz:Option[String] = None, options:Map[String, String] = Map.empty)(
    ...
    TableBuilder(ks, IndexEntry(name,internal.keyOf(column),clazz,options) +: indexes )
  }
  ...
  ): Table[R, PK, CK, IDX] = T.table(ks,name,options, self.indexes)
