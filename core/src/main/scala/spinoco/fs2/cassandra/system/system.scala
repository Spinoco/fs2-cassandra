...
package object system {
  ...
  def migrateKeySpace(desired:KeySpace, maybeCurrent: Option[KeyspaceMetadata]):Seq[String] = {
    maybeCurrent match {
      ...
      case Some(current) =>
        if (desired.name.toLowerCase != current.getName.toString.toLowerCase) Nil
        else {
  ...
  def samePrimaryKey2(currentPartitioning: List[ColumnMetadata], currentClustering: Map[ColumnMetadata, ClusteringOrder], desired: AbstractTable[_,_,_,_]):Boolean = {
    val currentPk = currentPartitioning.map(_.getName.toString.toLowerCase)
    val currentCk = currentClustering.keys.map(_.getName.toString.toLowerCase)
    ...
    maybeCurrent match {
      ...
      case Some(current) =>
        ...
        if (desiredTable.name != current.getName.asInternal) Nil
          ...
          val currentColumns = current.getColumns.asScala.map { c => c._1.toString.toLowerCase -> c._2.getType }
      ...
      case Some(current) =>
        ...
        def sameColumns: Boolean = {
          val currentColumns = current.getColumns.asScala
            .map { c => c._1.toString.toLowerCase -> c._2.getType }
            .toSeq
            .sortBy(_._1)
