...
sealed trait DMLStatement[I, O] extends CStatement[I] {
  ...
  def readResult(r: Option[Row], protocolVersion: ProtocolVersion): Either[Throwable, O]

  /** for batch results, this allows to read result related to this batched statement **/
...
trait Query[Q, R] extends CStatement[Q] { self =>
  ...
  def map[O](f: R => O): Query[Q, O] = {
    new Query[Q, O] {
      ...
      def read(r: Row, protocolVersion: ProtocolVersion): Either[Throwable, O] = self.read(r, protocolVersion).map(f)
...
trait Update[Q, R] extends DMLStatement[Q, R] { self =>
  ...
  def mapIn[B](f: B => Q): Update[B, R] = new Update[B, R]{
    ...
    def read(r: Row, protocolVersion: ProtocolVersion): Either[Throwable, R] = self.read(r, protocolVersion)
    def readResult(r: Option[Row], protocolVersion: ProtocolVersion): Either[Throwable, R] = self.readResult(r, protocolVersion)
  ...
  def map[B](f: R => B): Update[Q, B] = new Update[Q, B] {
    ...
    def cqlFor(q: Q): String = self.cqlFor(q)
    def writeRaw(q: Q, protocolVersion: ProtocolVersion): Map[String, ByteBuffer] = self.writeRaw(q, protocolVersion)
    def read(r: Row, protocolVersion: ProtocolVersion): Either[Throwable, B] = self.read(r, protocolVersion).map(f)
    def readResult(r: Option[Row], protocolVersion: ProtocolVersion): Either[Throwable, B] = self.readResult(r, protocolVersion).map(f)
    def fill(i: Q, s: PreparedStatement, protocolVersion: ProtocolVersion): BoundStatement = self.fill(i, s, protocolVersion)
...
trait Delete[Q, R] extends DMLStatement[Q, R] { self =>
  ...
  def map[R2](f: R => R2): Delete[Q, R2] = new Delete[Q, R2] {
    ...
    def cqlFor(q: Q): String = self.cqlFor(q)
    def writeRaw(q: Q, protocolVersion: ProtocolVersion): Map[String, ByteBuffer] = self.writeRaw(q, protocolVersion)
    def read(r: Row, protocolVersion: ProtocolVersion): Either[Throwable, R2] = self.read(r, protocolVersion).map(f)
    def readResult(r: Option[Row], protocolVersion: ProtocolVersion): Either[Throwable, R2] = self.readResult(r, protocolVersion).map(f)
    def fill(i: Q, s: PreparedStatement, protocolVersion: ProtocolVersion): BoundStatement = self.fill(i, s, protocolVersion)
...
trait Insert[I, O] extends DMLStatement[I, O] { self =>
  ...
  def mapIn[I2](f: I2 => I): Insert[I2, O] = {
    new Insert[I2, O] {
      ...
      def readResult(r: Option[Row], protocolVersion: ProtocolVersion): Either[Throwable, O] = self.readResult(r, protocolVersion)
  ...
  def map[O2](f: O => O2): Insert[I, O2] = {
    new Insert[I, O2] {
      ...
      def read(r: Row, protocolVersion: ProtocolVersion): Either[Throwable, O2] = self.read(r, protocolVersion).map(f)
      def readResult(r: Option[Row], protocolVersion: ProtocolVersion): Either[Throwable, O2] = self.readResult(r, protocolVersion).map(f)
